...
print(f"Rebuilding stock cache for {len(files)} files with full inlineStr support...")
...
def parse_file(fpath):
    ...
    with zipfile.ZipFile(fpath, 'r') as z:
        ...
        col_part_idx, col_desc_idx, col_brand_idx, col_cat_idx = 11, 13, 9, 10
        col_qty_idx, col_cost_idx, col_val_idx, col_loc_idx = 14, 15, 17, 3
        col_tag_idx, col_mrp_idx, col_age_idx = 7, 16, 20
            ...
            if elem.tag.endswith('row'):
                ...
                if r_num == 1:
                    for c_idx, h_text in row_cells.items():
                        ...
                        elif 'value' in h_l or 'valuation' in h_l: col_val_idx = c_idx
                        elif 'branch' in h_l: col_loc_idx = c_idx
                        elif 'tag' in h_l: col_tag_idx = c_idx
                        elif 'mrp' in h_l: col_mrp_idx = c_idx
                        elif 'agedays' in h_l or 'age' in h_l: col_age_idx = c_idx
                    elem.clear()
                    continue
                ...
                try:
                    ...
                    cat_name = str(row_cells.get(col_cat_idx, 'PRIMARY')).strip() or 'PRIMARY'
                    ...
                    if len(sample_items) < 200:
                        ...
                        sample_items.append({
                            ...
                            "valuation": v,
                            "ageDays": int(float(age_raw)) if age_raw else 0,
    ...
    t1 = time.time()
    print(f"Parsed {fname} ({date_str}) in {t1-t0:.2f}s | SKUs: {total_skus:,} | Val: INR {total_val/10000000:.2f} Cr | Cats: {list(cat_val.keys())[:5]}")
    return {
        "date": date_str,
        "filename": fname,
...
print("\n[SUCCESS] Rebuilt stock cache with full DoD Category Variance!")
print(f"Latest Date: {latest_d} | Categories: {[item['category'] for item in dod_category_variance]}")
