...
def main():
    ...
    print(f"Building PERFECT ULTRA-FAST Stock Cache for {len(files)} files...")
    ...
    if latest_d and prev_d and latest_d in daily_summaries and prev_d in daily_summaries:
        ...
        CAT_ORDER = {'OEM': 1, 'PRIMARY': 2, 'SECONDARY': 3, 'PL': 4, 'CASTROL': 5, 'LUBES': 6, 'PAINT&CONS': 7, 'ASSET': 8, 'UNCATEGORIZED': 999}
        all_cats = sorted(list(set(list(l_cats.keys()) + list(p_cats.keys()))), key=lambda c: (CAT_ORDER.get(c.upper(), 500), c))
    ...
    t_end = time.time()
    print(f"\n[COMPLETE SUCCESS] Pure clean stock cache generated in {t_end - t_start:.2f} seconds for dates: {dates_list}")
