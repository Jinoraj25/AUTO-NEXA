...
month_date_map = {
    'JAN': '2026-01-15', 'FEB': '2026-02-15', 'MAR': '2026-03-15', 'APR': '2026-04-15',
    'MAY': '2026-05-15', 'JUN': '2026-06-15', 'JUL': '2026-07-15', 'AUG': '2026-08-15'
}
...
def run():
    print("=================================================================")
    print("  MONTH-WISE SALES UPLOAD TO MYSQL (RF & CF JAN '26 - AUG '26)   ")
    print("=================================================================")
    ...
    print(f"\n📁 1. Reading RF Sales Spreadsheet: {RF_FILE}")
    t0 = time.time()
    df_rf = pd.read_excel(RF_FILE)
    print(f"   -> Loaded {len(df_rf):,} raw RF rows in {time.time()-t0:.2f}s.")

    # RF Month parsing (datetime column)
    df_rf['DateParsed'] = pd.to_datetime(df_rf.get('Month'), errors='coerce')
    df_rf['MonthKey'] = df_rf['DateParsed'].dt.strftime('%b').str.upper()
    df_rf['DateInv'] = pd.to_datetime(df_rf.get('InvoiceDate'), errors='coerce')
    df_rf['MonthKey'] = df_rf['MonthKey'].fillna(df_rf['DateInv'].dt.strftime('%b').str.upper()).fillna('AUG')

    # 3. Process CF Sales File
    print(f"\n📁 2. Reading CF Sales Spreadsheet: {CF_FILE}")
    t1 = time.time()
    ...
    # CF Month parsing (Excel serial number column)
    df_cf['Num'] = pd.to_numeric(df_cf.get('Month'), errors='coerce')
    df_cf['DateParsed'] = pd.to_datetime(df_cf['Num'], origin='1899-12-30', unit='D', errors='coerce')
    df_cf['MonthKey'] = df_cf['DateParsed'].dt.strftime('%b').str.upper().fillna('AUG')
    ...
    print("\n=================================================================")
    print("  COMMITTING MONTH BY MONTH INTO MYSQL (`daily_sales_consolidated`)")
    print("=================================================================")

    total_uploaded = 0
    total_revenue_cr = 0.0
    ...
    for m in months_order:
        ...
        for r in rf_m.itertuples():
            d_val = getattr(r, 'DateParsed', getattr(r, 'DateInv', None))
            tx_d = d_val.strftime('%Y-%m-%d') if (d_val is not None and pd.notnull(d_val)) else month_date_map[m]
            ...
            m_rows.append((
                ...
                clean_str(getattr(r, 'Region', 'SOUTH'), 'SOUTH').upper(), clean_str(getattr(r, 'Remarks', 'RF Daily Sales Dump'), 'RF Daily Sales Dump')
            ))

        # CF rows for month m
        for r in cf_m.itertuples():
            d_val = getattr(r, 'DateParsed', None)
            tx_d = d_val.strftime('%Y-%m-%d') if (d_val is not None and pd.notnull(d_val)) else month_date_map[m]
        ...
        m_cnt = len(m_rows)
        total_uploaded += m_cnt
        total_revenue_cr += (m_rev / 1e7)

        print(f"\n▶ Uploaded Month: {m:3s} (2026)")
        print(f"   • RF Rows: {len(rf_m):7,} | CF Rows: {len(cf_m):7,} | Total Month Items: {m_cnt:7,}")
        print(f"   • Month Sales Valuation: ₹{m_rev/1e7:6.2f} Cr (₹{m_rev:,.2f})")
    ...
    print("\n=================================================================")
    print(f"🎉 ALL 8 MONTHS (JAN TO AUG) SUCCESSFULLY UPLOADED TO MYSQL!")
    print(f"   • Total Sales Rows in MySQL: {total_uploaded:,} rows")
    print(f"   • Total Revenue Consolidated: ₹{total_revenue_cr:.2f} Cr")
    print("=================================================================")
